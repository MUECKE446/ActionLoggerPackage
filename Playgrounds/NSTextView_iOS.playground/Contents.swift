...
#if os(OSX)
let testView = NSView(frame: CGRect(x: 0.0, y: 0.0, width: 500.0, height: 667.0))
let frame = testView.frame
let contentSize = CGSizeMake(frame.width, frame.height)
let textView: NSTextView = NSTextView(frame: NSMakeRect(0, 0, contentSize.width, contentSize.height))
#endif
#if os(iOS)
let testView = UIView(frame: CGRect(x: 0.0, y: 0.0, width: 500.0, height: 667.0))
let frame = testView.frame
let contentSize = CGSizeMake(frame.width, frame.height)
let textView: UITextView = UITextView(frame: CGRectMake(0, 0, contentSize.width, contentSize.height))
#endif

testView.addSubview(textView)

let textViewDestination = ActionLogTextViewDestination(identifier:"textView",textView: textView)
let log = ActionLogger(identifier: "newLogger",logDestinations: [textViewDestination])
log!.info("this is the first trial")
log!.verbose("better as expected")
log!.error("this could be an error message")
